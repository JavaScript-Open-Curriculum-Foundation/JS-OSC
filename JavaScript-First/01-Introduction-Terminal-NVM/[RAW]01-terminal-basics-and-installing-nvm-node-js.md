# Terminal Basics & Installing NVM
...
`ls / dir` , `pwd`, and `cd` are all commands that are useful for navigating system files & folders (directories). `pwd` will print the working directory and show you the full path to the directory you are in. `dir` & `ls` list directory contents but `dir` is **used on window**s where as ls is used on linux / mac. cd will allow you to change the directory you are in. ls & cd are the two you will use the most, so learn them well.
...
If it doesn’t exist let's go ahead and create one with touch ~/.bash_profile.
...
Then consider becoming a [github sponsor](https://github.com/sponsors/HansUXdev). I worked as a high school coding teacher before COVID and I want to do this full time so I can spend more time with my son who's at high risk.

If you can afford Netflix or a course on udemy, etc, then can probably afford to fund Open Source Projects. You'll also help fund fun coding challenges such as **#CodeRedChallenge**. 

Click the image below to [read more about the challenge](https://github.com/HansUXdev/OSS-Books/tree/master/Code-Red-Challenge):
...
Thank you for reading all this way and I hope you enjoyed it. I plan to update every article in the series with links to the next article and have them available on [github](https://github.com/HansUXdev/OSS-Books), [medium](https://medium.com/@HansOnConsult), [dev.to](https://dev.to/hansuxdev) and [codementor.io](https://www.codementor.io/@hansdev).


#### What do you really know about Variables, Data Types and Immutability in Javascript?
In this article we'll go over a gentle (re)introduction to variables, data types and immutability. It’s intended to be resource for beginners and advanced students alike and provide readers with rock-solid foundation essential to learning javascript with ES6 built in as a standard rather than a stand alone buzzword topic after 5+ years...
